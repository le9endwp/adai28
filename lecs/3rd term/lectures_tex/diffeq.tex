...
\section{Лекция 9. Жордановы все на свете}

\begin{nota}{Зачем нужны экспоненты матриц?}
    Если мы имеет систему $x' = Ax$, где $A$ -- постоянная матрица, то одной из фундаментальных матриц будет $\Phi(t) := e^{At}$

    $e^{At} = I + At + \frac{A^2t^2}{2} + \ldots + \frac{A^nt^n}{n!} + \ldots$

    $\frac{d}{dt}e^{At} = A + A^2t + \ldots + \frac{A^{n + 1}t^n}{n!} + \ldots = Ae^{At}$

    В этом случае общее решение нашей системы имеет вид $e^{At}C$, где $C$ -- постоянный вектор-столбец
\end{nota}

\begin{Reminder}{Жордановы нормальные формы}
    Напоминание из алгебры

    \textbf{Факт.} Пусть $A$ -- некоторая квадратная матрица $\lambda_1, \lambda_2, \ldots, \lambda_n$ -- собственные числа. Тогда существует единственная с точностью до перестановок блоков блочно-диагональная матрица 
    ...
    $J = \begin{pmatrix}
        ...
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & J_m
    \end{pmatrix}$, где каждая матрица $J_k = \begin{pmatrix}
        \lambda_k & 1 & 0 & \ldots & 0 \\
        0 & \lambda_k & 1 & \ldots & 0 \\
        \vdots & \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & 0 & \ldots & 1 \\
        0 & 0 & 0 & \ldots & \lambda_k
    \end{pmatrix}$, \\ такая, что $J = SAS^{-1}$, где $S$ -- некоторая невырожденная матрица. Вместо единичек могут быть любые числа 
\end{Reminder}

\begin{Remark}{}
    Собственные числа $\lambda_1, \ldots, \lambda_n$ могут повторяться, а также быть мнимыми, равно как и матрицы $J$ и $S$
\end{Remark}

\begin{nota}{Как строится матрица $S$?}
    В простейшем случае это может быть матрица из собственных векторов, записанных в том же порядке, что и соответствующие собственные числа 

    В случае, если есть собственный вектор и отвечающая ему цепочка дополнительных векторов, выписываем их по порядку справа налево (для единичек сверху -- слева направо)
\end{nota}
...
\begin{Example}{}
    Пусть $A = \begin{pmatrix}
        ...
        4 & 1 & 0 \\
        2 & 1 & -1
    \end{pmatrix}$, собственные числа 1 и -1 кратности 2

    $A - \Id = \begin{pmatrix}
        -2 & 1 & -2 \\
        4 & 0 & 0 \\
        2 & 1 & -2
    \end{pmatrix}$, собственный вектор для 1 равен $u_0 = \begin{pmatrix}
        0 \\ 
        2 \\
        1
    \end{pmatrix}$
    ...
    $A + \Id = \begin{pmatrix}
        ...
        4 & 2 & 0 \\
        2 & 1 & 0
    \end{pmatrix}$ -- ранг равен 2, собственный вектор только 1

    $(A + \Id)^2 = \begin{pmatrix}
        0 & 0 & 0 \\
        8 & 8 & -8 \\
        4 & 4 & -4
    \end{pmatrix}$. Ранг уже равен 1. Возьмем собственный вектор этой матрицы, не являющийся собственным вектором матрицы $A + \Id$

    Например, вектор $v_1 = \begin{pmatrix}
        0 \\
        1 \\
        1
    \end{pmatrix}$. Ему будет отвечать собственный вектор 
    
    $v_0 = (A + \Id)v_1 = \begin{pmatrix}
        -1 \\
        2 \\
        1
    \end{pmatrix}$ матрицы $A$ для собственного числа $-1$

    Итого, жорданова форма имеет вид $J = \begin{pmatrix}
        1 & 0 & 0 \\
        0 & -1 & 0 \\
        0 & 1 & -1
    \end{pmatrix}$, 
    
    а матрица перехода $S = \begin{pmatrix}
        0 & 0 & -1 \\
        2 & 1 & 2 \\
        1 & 1 & 1
    \end{pmatrix}$
...
\begin{Example}{}
    Пусть $A = \begin{pmatrix}
        ...
        2 & -1 & -2 \\
        -1 & 1 & 2
    \end{pmatrix}$, собственное число 1 кратности 3

    Имеем $A - \Id = \begin{pmatrix}
        1 & -1 & -1 \\
        2 & -2 & -2 \\
        -1 & 1 & 1
    \end{pmatrix}$ ранг 1. Тогда $(A - \Id)^2 = 0$

    Выберем вектор, обнуляемый $(A - \Id)^2$ (то есть любой), не обнуляемый матрицей $A - \Id$
    ...
    $v_1 = \begin{pmatrix}
        ...
        0 \\
        0
    \end{pmatrix}$. Ему соответствует собственный вектор матрицы $A - I$, равный $\begin{pmatrix}
        1 \\
        2 \\
        -1
    \end{pmatrix}$
    
    Возьмем еще один собственный вектор этой матрицы, независимый с предыдущим. Например, $\begin{pmatrix}
        1 \\
        1 \\
        0
    \end{pmatrix}$. Ему будет соответствовать отдельная клетка в жордановой форме 

    Выписываем жорданову форму 
    
    $J = \begin{pmatrix}
        1 & 0 & 0 \\
        1 & 1 & 0 \\
        0 & 0 & 1
    \end{pmatrix}$ и матрицу перехода $A = S = \begin{pmatrix}
        1 & -1 & 1 \\
        0 & 2 & 1 \\
        0 & -1 & 0
    \end{pmatrix}$
...
\begin{nota}{}
    Если матрицы $A$ и $B$ сопряженные, то есть $A = SBS^{-1}$, то для любого $n \in \N$ $A^n = SB^nS^{-1}$. Тогда, подставляя это соотношение в формулу $e^A := \Id + \sum\limits_{n = 1}^\infty \frac{A^n}{n!}$, получаем $e^A = Se^BS^{-1}$

    Таким образом, достаточно научиться считать экспоненты только от жоржановых форм. Степени (а значит, и экспоненты) от блочно-диагональных матриц считаются поблочно 

    Значит, достаточно научиться считать экспоненты от жордановых блоков 

    Каждый жорданов блок имеет вид $J_k = \begin{pmatrix}
        \lambda_k & 0 & \ldots & 0 \\
        1 & \lambda_k & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & \lambda_k
    \end{pmatrix}$ (пусть это матрица $m \times m$) и, значит, может быть записан в виде $J = \lambda_k \Id + U$, где $U = \begin{pmatrix}
        0 & 0 & \ldots & 0 \\
        1 & 0 & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & 0
    \end{pmatrix}$. В этой матрице все элементы, кроме поддиагональных, нулевые. В матрице $U^2$ единички спустятся на диагональ ниже, потом ниже, матрица $U^{m - 1}$ будет иметь единичку в левом нижнем углу и, наконец $U^m = 0$
\end{nota}

\begin{nota}{}
    Так как $J_k = \lambda_k\Id + U$, то $e^{Jt} = e^{\Id\lambda t} e^{Ut}$ (матрицы коммутируют)

    С другой стороны, $e^{\Id\lambda t} = e^{\lambda t}\Id$. Осталось посчитать

    $e^{Ut} = \Id + \sum\limits_{n = 1}^\infty \frac{U^nt^n}{n!} = \begin{pmatrix}
        1 & 0 & 0 & \ldots & 0 & 0 \\
        t & 1 & 0 & \ldots & 0 & 0 \\
        \frac{t^2}{2} & t & 1 & \ldots & 0 & 0 \\
        \vdots & \vdots & \vdots & \ddots & \vdots & \vdots \\
        \frac{t^{m - 2}}{(m - 2)!} & \frac{t^{m - 3}}{(m - 3)!} & \frac{t^{m - 4}}{(m - 4)!} & \ldots & 1 & 0 \\
        \frac{t^{m - 1}}{(m - 1)!} & \frac{t^{m - 2}}{(m - 2)!} & \frac{t^{m - 3}}{(m - 3)!} & \ldots & t & 1
    \end{pmatrix}$

    Тогда $e^{J_kt} = \begin{pmatrix}
        e^{\lambda_k t} & 0 & 0 & \ldots & 0 & 0 \\
        t e^{\lambda_k t} & e^{\lambda_k t} & 0 & \ldots & 0 & 0 \\
        \frac{t^2}{2} e^{\lambda_k t} & t e^{\lambda_k t} & e^{\lambda_k t} & \ldots & 0 & 0 \\
        \vdots & \vdots & \vdots & \ddots & \vdots & \vdots \\
        \frac{t^{m - 2}}{(m - 2)!} e^{\lambda_k t} & \frac{t^{m - 3}}{(m - 3)!} e^{\lambda_k t} & \frac{t^{m - 4}}{(m - 4)!} e^{\lambda_k t} & \ldots & e^{\lambda_k t} & 0 \\
        \frac{t^{m - 1}}{(m - 1)!} e^{\lambda_k t} & \frac{t^{m - 2}}{(m - 2)!} e^{\lambda_k t} & \frac{t^{m - 3}}{(m - 3)!} e^{\lambda_k t} & \ldots & t e^{\lambda_k t} & e^{\lambda_k t}
    \end{pmatrix}$
\end{nota}

\begin{Example}{}
    Решим систему $\begin{cases}
        x' = 2x + y \\
        y' = 3x + 4y
    \end{cases}$
    ...
    $A = \begin{pmatrix}
        2 & 1 \\
        3 & 4
    \end{pmatrix},\ \det A = 5,\ \Tr A = 6,\ \lambda_1 = 1,\ \lambda_2 = 5$
    ...
    \end{pmatrix},\ u_1 = \begin{pmatrix}
        1 \\
        -1
    \end{pmatrix};\ A - 5\Id = \begin{pmatrix}
        -3 & 1 \\
        3 & -1
    ...
    \end{pmatrix};\ e^{Jt} = \begin{pmatrix}
        e^t & 0 \\
        0 & e^{5t}
    \end{pmatrix},\ \Phi(t) = Se^{Jt} = \begin{pmatrix}
        e^t & e^{5t} \\
        -e^t & 3e^{5t}
    \end{pmatrix}$

    \textbf{Ответ:} $x = C_1e^t + C^2e^{5t},\ y = -C_1e^t + 3C_2e^{5t}$
\end{Example}

\begin{Example}{}
    Решаем систему $\begin{cases}
        x' = -x + y - 2z \\
        y' = 4x + y \\
        z' = 2x + y - z 
    \end{cases}$
    ...
    \textbf{Ответ:} $\begin{cases}
        x = -C_2te^{-t} - C_3e^{-t} \\
        y = 2C_1e^t + C_2(1 + 2t)e^{-t} + 2C_3e^{-t} \\
        z = C_1e^t + C_2(1 + t)e^{-t} + C_3e^{-t}
    \end{cases}$
\end{Example}

\begin{defin}{Вещественная жоржанова форма}
    \textbf{Факт из алгебры.} Если у вещественной матрицы есть два сопряженных мнимых собственных числа $\lambda = \alpha + i\beta$ и $\ol{\lambda} = \alpha - i\beta$, то собственные и присоединенные вектора также могут быть выбраны сопряженными, количество жордановых клеток одинаково и они имеют одинаковую структуру. $Au = \lambda u \Rightarrow A\ol{u} = \ol{\lambda}\ol{u}$
\end{defin}

\begin{theo}{}
    Пусть $A$ -- некоторая вещественная матрица, $\mu_1, \ldots, \mu_k \in \R$, $\lambda_1, \ol{\lambda_1}, \ldots, \lambda_m, \ol{\lambda_m} \notin \R$ -- собственные числа, $\lambda_j = \alpha_j + i\beta_j$. Тогда существует единственная с точностью до перестановок блоков матрица $J = \begin{pmatrix}
        J_1 & 0 & \ldots & 0 \\
        0 & J_2 & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & J_p
    \end{pmatrix}$, где каждая матрица $J_k$ имеет вид или $\begin{pmatrix}
        \mu_j & 0 & \ldots & 0 \\
        1 & \mu_j & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & \mu_j
    \end{pmatrix}$, или $\begin{pmatrix}
        \Lambda_j & 0 & \ldots & 0 \\
        E_2 & \Lambda_j & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & \Lambda_j
    \end{pmatrix}$, где $\Lambda_j = \begin{pmatrix}
        \alpha_j & \beta_j \\
        -\beta_j & \alpha_j
    \end{pmatrix},\ E_2 = \begin{pmatrix}
        1 & 0 \\
        0 & 1
    \end{pmatrix}$, такая, что $J = SAS^{-1}$, где $S$ -- некоторая невырожденная вещественная матрица (заметим, что вместо единичек, как в вещественных, так и в комплексных блоках можно ставить любое ненулевое число $\varepsilon$)
\end{theo}

\begin{nota}{Метод построения вещественной жордановой формы}
    Пусть для заданная матрица $A = SJS^{-1}$, где $J$ -- обычная жорданова форма, $S$ -- матрица перехода. Пусть в матрице $S$ есть последовательность из собственного и дополнительных векторов $u_{m - 1}, \ldots, u_1, u_0$, отвечающая жоржановой клетке $J_j$ для мнимого собственного числа $\lambda_j$. Тогда будет и последовательность из собственного и дополнительных векторов $\ol{u_{m - 1}}, \ldots, \ol{u_1}, \ol{u_0}$, отвечающая жордановой клетке $\ol{J_j}$ для мнимого собственного числа $\ol{\lambda_j}$. Пусть $u_k = v_k + iw_k$. Тогда в матрице перехода к вещественной жордановой форме в тех столбцах, где стоит блок $\begin{pmatrix}
        \Lambda_j & 0 & \ldots & 0 \\
        E_2 & \Lambda_j & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & \Lambda_j
    \end{pmatrix}$, ставим набор $(v_{m - 1}, w_{m - 1}, \ldots, v_1, w_1, v_0, w_0)$ -- именно в таком порядке
\end{nota}

\begin{nota}{Факт из алгебры}
    Матрицы вида $\Lambda = \begin{pmatrix}
        \alpha & \beta \\
        -\beta & \alpha
    \end{pmatrix}$ образуют поле, изоморфное $\C$

    Соответственно, $e^{\Lambda t} = \begin{pmatrix}
        e^{\alpha t} \cos \beta t & e^{\alpha t} \sin \beta t \\
        -e^{\alpha t} \sin \beta t & e^{\alpha t} \cos \beta t
    \end{pmatrix}$

    Экспонента от вещественно жордановой формы считается, так же, как и в обычном случае. Единственная разница: когда имеем дело с блоком или $J_j = \begin{pmatrix}
        \Lambda_j & 0 & \ldots & 0 \\
        E_2 & \Lambda_j & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        0 & 0 & \ldots & \Lambda_j
    \end{pmatrix}$, то имеем $e^{J_jt} = \begin{pmatrix}
        e^{\Lambda_j t} & 0 & \ldots & 0 \\
        te^{\Lambda_j t} & e^{\Lambda_j t} & \ldots & 0 \\
        \vdots & \vdots & \ddots & \vdots \\
        \frac{t^{m - 1}}{(m - 1)!} e^{\Lambda_j t} & \frac{t^{m - 2}}{(m - 2)!} e^{\Lambda_j t} & \ldots & e^{\Lambda_j t}
    \end{pmatrix}$
\end{nota}

\begin{Remark}{Зачем это все нужно?}
    Решаем систему $x' = Ax$, знаем $e^{At}$ -- фундаментальная матрица. Тогда $e^{At} = Se^{Jt}S^{-1}$. В качестве фундаментальной можем взять матрицу $\Phi(t) := Se^{Jt}$. Если $J$ -- вещественная жорданова форма, то матрица $\Phi(t)$ тоже вещественна 
\end{Remark}
...
\begin{Example}{}
    ...
    \end{vmatrix} + \begin{vmatrix}
        3 & -2 - \lambda \\
        -1 & 2
    \end{vmatrix} = \\ = (3 - \lambda)\lambda^2 + 2\lambda - 4 + 10 - 10\lambda = -\lambda^3 + \lambda^2 - 2 = 0$

    Характеристическое уравнение имеет вид $\lambda^3 - \lambda^2 + 2 = 0$, собственные числа равны $-1$ и $1 \pm i$

    Координаты собственного вектора для $\lambda_1 = -1$ удовлетворяют системе $A - \lambda_1E, \overrightarrow{\alpha} = 0$, или $\begin{pmatrix}
        4 & -3 & 1 \\
        3 & -1 & 2 \\
        -1 & 2 & 1
    \end{pmatrix} \begin{pmatrix}
    ...
    \end{pmatrix} = \begin{pmatrix}
        ...
        0 \\
        0
    \end{pmatrix}$. Отсюда сам вектор равен $\begin{pmatrix}
        1 \\
        1 \\
        -1
    \end{pmatrix}$

    Теперь найдем собственный вектор для $\lambda_2 = 1 + i$ (для $1 - i$ вектор будет сопряженный)

    Получаем систему $A - \lambda_2E, \overrightarrow{\gamma} = 0,\ \begin{pmatrix}
        2 - i & -3 & 1 \\
        3 & -3 - i & 2 \\
        -1 & 2 & -1 - i
    \end{pmatrix} \begin{pmatrix}
    ...
    Прибавляя ко второму уравнению три раза третье, получим $(3 - i)\gamma_2 - (1 + 3i)\gamma_3 = 0$. Можно взять $\gamma_2 = 1 + 3i,\ \gamma_3 = 3 - i$
    
    Из третьего уравнения $\gamma_1 = 2\gamma_2 - (1 + i)\gamma_3 = 2 + 6i - 3 + i - 3i + 1 = 4i \Rightarrow \begin{pmatrix}
        4i \\
        1 + 3i \\
        3 - i
    \end{pmatrix}$
    ...
    $\Phi(t) = Se^{Jt} = \begin{pmatrix}
        e^{-t} & -4e^t \sin t & 4e^t \cos t \\
        e^{-t} & e^t (\cos t - 3\sin t) & e^t (\sin t + 3\cos t) \\
        -e^{-t} & e^t (3\cos t + \sin t) & -e^t(\cos t + 3\sin t)
    \end{pmatrix}$. Первый столбец: $e^{\lambda t}u$

    $\begin{cases}
        x = C_1e^{-t} - 4C_2e^t \sin t + 4C_3e^t \cos t \\
        y = C_1e^{-t} + C_2e^t (\cos t - 3\sin t) + C_3e^t (\sin t + 3\cos t) \\
        z = -C_1e^{-t} + C_2e^t (3\cos t + \sin t) - C_3e^t (\cos t + 3\sin t)
    \end{cases}$
\end{Example}

\begin{nota}{Лайфхак}
    Будет комплексное решение $e^{(1 + i)t} = e^t \begin{pmatrix}
        -4\sin t \\
        \cos t - 3\sin t \\
        3\cos t + \sin t
    ...
    Вещественная и внимая часть решения дадут второй и третий столбец фундаментальной матрицы
\end{nota}

\newpage 

\section{Лекция 10. Классификация Пуанкаре}

\begin{nota}{Классификация Пуанкаре}
    В этом разделе мы представим классификацию точек покоя
    ...
    \end{pmatrix} \begin{pmatrix}
        x \\ 
        y 
    \end{pmatrix}$, где $A \in \R^{2 \times 2}$. Предполагаем, что $\det A \neq 0$

    Если у нас собственные числа вещественные, то $A = SJS^{-1}$ и замена $\zeta = S^{-1}z,\ z = S\zeta,\\ z' = S\zeta' = Az = AS\zeta,\ \zeta' = S^{-1}AS\zeta = J\zeta$

    Если $\lambda_{1, 2}$ -- собственные числа и жорданова форма без единичек, то система примет вид $\begin{cases}
        \xi' = \lambda_1 \xi \\
        \eta' = \lambda_2 \eta
    \end{cases},\ \zeta = \begin{pmatrix}
        \xi \\
        \eta
    \end{pmatrix}.\ \begin{cases}
        \xi = C_1e^{\lambda_1 t} \\
        \eta = C_2e^{\lambda_2 t}
    \end{cases},\ \eta = C_2 \left(\frac{\xi}{C_1}\right)^\frac{\lambda_2}{\lambda_1}$, при $C_1 \neq 0$
\end{nota}

\begin{defin}{Узел}
    Если $\lambda_1\lambda_2 > 0$ (то есть $\det A > 0$) $|\lambda_1| < |\lambda_2|$, то имеем устойчивый узел при $\lambda_1 < \lambda_2 < 0$ и неустойчивый узел при $0 < \lambda_1 < \lambda_2$

    Пока что (потом будет дано точно определение) под устойчивостью понимаем то, что решения с любыми начальными данными остаются ограниченными на положительной полуоси
\end{defin}

\begin{defin}{Дикритические узлы}
    Если $\lambda_1 = \lambda_2 = \lambda \in \R$ и у матрицы $A$ есть два линейно независимых собственных вектора (а это значит, что любой вектор является собственным для матрицы $A = \lambda \Id$), имеем дело с так называемым дикритическим узлом, который устойчив, при $\lambda_1 = \lambda_2 < 0$ и неустойчив при $\lambda_1 = \lambda_2 > 0$ 
\end{defin}

\begin{defin}{Вырожденные узлы}
    Если $\lambda_1 = \lambda_2 = \lambda \in \R$ и матрица $A$ имеет одномерное пространство собственных векторов, $\begin{cases}
        \xi\ = \lambda \xi \\
        \eta' = \xi + \lambda \eta 
    \end{cases},\ \begin{cases}
        \xi = C_1 e^{\lambda t} \\ 
        \eta = C_1 t e^{\lambda t} + C_2 e^{\lambda t}
    \end{cases},\ \eta = \frac{C_2}{C_1} \xi + \frac{\xi}{\lambda} \ln{\frac{\xi}{C_1}}$, при $C_1 \neq 0$ картинка называется вырожденным узлом, устойчивый при $\lambda_1 = \lambda_2 < 0$ и неустойчивый при $\lambda_1 = \lambda_2 > 0$
\end{defin}

\begin{defin}{Седло}
    Если $\lambda_1 < 0 < \lambda_2$, то имеет место так называемое седло (всегда неустойчивое). Две траектории втыкаются в ноль, две выходят из нуля, остальные выглядят как гиперболы
\end{defin}

\begin{Remark}{Случай комплексных собственных чисел}
    Как мы знаем, вещественная жорданова форма в случае комплексных собственных чисел имеет вид $\begin{pmatrix}
        \alpha & \beta \\
        -\beta & \alpha
    \end{pmatrix}$

    Соответственно, в координатах $\zeta$, система примет вид $\begin{cases}
        \xi' = \alpha \xi + \beta \eta \\
        \eta' = -\beta \xi + \alpha \eta
    \end{cases}$

    Применим полярную замену координат $\xi = r\cos\varphi,\ \eta = r\sin\varphi$. Тогда в системе получим $\begin{cases}
        r'\cos\varphi - r\sin\varphi\varphi' = \alpha r\cos\varphi + \beta r\sin\varphi \\
        r'\sin\varphi + r\cos\varphi\varphi' = -\beta r\cos\varphi + \alpha r\sin\varphi
    \end{cases}$

    Отсюда, комбинируя уравнения выше, получаем $\begin{cases}
        r' = \alpha r \\
        \varphi' = -\beta 
    \end{cases} \Rightarrow \begin{cases}
        r = C_1 e^{\alpha t} \\
...
\begin{defin}{Центры}
    Если $\lambda_1 = \ol{\lambda_2} \in i\R$, то есть $\alpha = \re \lambda_1 = \re \lambda_2 = 0$, то точка покоя имеет тип центр 

    У центра есть направление закрутки, против либо против часовой стрелки. Это можно определить сразу из вида системы еще до всяких переходов к жордановой форме: если коэффициент $c$ в системе положителен, то закрутка против часовой стрелки, а если отрицателен, то по. Равен нулю этот коэффициент быть не может. Можно определить закрутку и по коэффициенту $b$ (будет наоборот)
\end{defin}

\begin{defin}{Фокусы}
    Точка покоя называется устойчивым фокусом в случае $\re \lambda_1 = \re \lambda_2 < 0$ и неустойчивым фокусом, если $\re \lambda_1 = \re \lambda_2 > 0$

    Независимо от устойчивости фокус тоже может быть закручен по часовой стрелке или против. Направление закрутки определяется также, как и для центра
\end{defin}

\begin{defin}{Линейные неоднородные системы}
    Решаем линейную неоднородную систему $x' = A(t)x + f(t)$ и, отвечающую ей однородную систему $x' = A(t)x$

    Пусть $\Phi(t)$ -- фундаментальная матрица системы. Тогда общее решение системы имеет вид $x(t) = \Phi(t)C$, где $C = \begin{pmatrix}
        c_1 \\
        \ldots \\
        c_n 
    \end{pmatrix}$. При этом, общее решение системы имеет вид \\ $x(t) = \Phi(t)C + x^*(t)$, где $x^*(t)$ -- некоторое частное решение системы. Доказывается также, как и для линейных уравнений -- берем разность двух решений системы 
\end{defin}

\begin{defin}{Принцип суперпозиции}
    Пусть есть две системы: $x' = A(t)x + f_1(t)$ и $x' = A(t)x + f_2(t)$, имеющие решения $x_1^*(t)$ и $x_2^*(t)$ соответственно. Тогда система $x' = A(t)x + af_1(t) + bf_2(t)$ имеет решение $ax_1^*(t) + bx_2^*(t)$
\end{defin}

\begin{nota}{Метод вариации произвольной постоянной}
    Частное решение $x^*(t)$ системы $x' = A(t)x + f(t)$ ищем в виде $x^*(t) = \Phi(t)C(t)$, где $C(t) = \begin{pmatrix}
        c_1(t) \\
        \ldots \\
        c_n(t)
    \end{pmatrix}$ некоторая дифференцируемая векторная функция 

    Подставляя выражение в уравнение, получаем $\Phi'(t)C(t) + \Phi(t)C'(t) = A(t)\Phi(t)C(t) + f(t)$, а поскольку $\Phi(t)$ удовлетворяет уравнению $\Phi'(t) = A(t)\Phi(t)$, имеем $\Phi(t)C'(t) = f(t)$

    Отсюда следует, что решение системы имеет вид выражения, где $C(t)$ -- функция, удовлетворяющая уравнению. Это уравнение, то есть система с неизвестными $c_1'(t), \ldots, c_n'(t)$ имеет единственное решение, поскольку $\det \Phi \neq 0$. Это решение имеет вид $C'(t) = \Phi^{-1}(t)f(t)$, что после интегрирования дает $C(t) = C(t_0) + \int\limits_{t_0}^t \Phi^{-1}(s)f(s)ds$

    При наличии начального условия $x(t_0) = x_0$, из уравнения получаем $x_0 = \Phi(t_0)C(t_0)$, то есть $C(t_0) = \Phi^{-1}(t_0)x_0$

    Тогда $C(t) = \Phi^{-1}(t_0)x_0 + \int\limits_{t_0}^t \Phi^{-1}(s)f(s)ds$

    Умножая это равенство на $\Phi(t)$, получаем $x(t) = \Phi(t)\Phi^{-1}(t_0)x_0 + \Phi(t)\int\limits_{t_0}^t \Phi^{-1}(s)f(s)ds$
\end{nota}

\begin{theo}{}
    Решение задачи Коши $x' = A(t)x + f(t),\ x(t_0) = x_0$ находится по формуле выше 

    Для систем с постоянными коэффициентами удобной эквивалентной формой будет $x(t) = e^{A(t - t_0)}x_0 + \int\limits_{t_0}^t e^{A(t - s)}f(s)ds$
\end{theo}
...
\begin{Example}{}
    ...
    $A = \begin{pmatrix}
        3 & -2 \\
        2 & -1
    \end{pmatrix},\ \det A = 1,\ \Tr A = 2,\ \lambda_1 = \lambda_2 = 1$

    $A - \Id = \begin{pmatrix}
        2 & - 2 \\
        2 & -2
    \end{pmatrix}$. Собственный вектор $\begin{pmatrix}
        2 \\ 
        2
    \end{pmatrix}$, соответствующий ему присоединенный вектор $\begin{pmatrix}
        1 \\
        0
    \end{pmatrix}$. Жорданова форма $J = \begin{pmatrix}
        1 & 0 \\
        1 & 1
    \end{pmatrix},\ \begin{pmatrix}
        2 & -2 \\
    ...
    \end{pmatrix} = \begin{pmatrix}
        2 \\
        2
    \end{pmatrix},\ (A - \Id)u_1 = u_0$

    Матрица перехода $S = \begin{pmatrix}
        1 & 2 \\
        0 & 2
    \end{pmatrix}$. Тогда $e^{Jt} = \begin{pmatrix}
        e^t & 0 \\
        te^t & e^t
    \end{pmatrix}$

    Фундаментальная матрица $\Phi(t) = Se^{Jt} = \begin{pmatrix}
        (2t + 1)e^t & 2e^t \\
        2te^t & 2e^t
    \end{pmatrix}$
    ...
    \end{pmatrix} = \begin{pmatrix}
        1 & -1 \\
        0 & \frac{1}{2}
    \end{pmatrix};\ e^{At} = \Phi(t)S^{-1} = \begin{pmatrix}
        (2t - 1)e^t & -2te^t \\
        2te^t & (1 - 2t)e^t
    ...
    $e^{A(t - s)}f(s) = \begin{pmatrix}
        -30(t - s)e^{t - s}e^s\sqrt{s} \\
        15(1 - 2(t - s))e^{t - s}e^s\sqrt{s}
    \end{pmatrix} = e^t \begin{pmatrix}
        -30t\sqrt{s} + 30s\sqrt{s} \\
        15\sqrt{s} - 30t\sqrt{s} + 30s\sqrt{s}
    \end{pmatrix}$

    $\int\limits_0^t e^{A(t - s)}f(s)ds = e^t \begin{pmatrix}
        -20t^\frac{5}{2} + 12t^\frac{5}{2} \\
        10t^\frac{3}{2} - 20t^\frac{5}{2} + 12t^\frac{5}{2}
    \end{pmatrix} = e^t \begin{pmatrix}
        -8t^\frac{5}{2} \\
        10t^\frac{3}{2} - 8t^\frac{5}{2}
    \end{pmatrix}$
    ...
    \textbf{Ответ:} $\begin{cases}
        x(t) = C_1(2t + 1)e^t 2C_2e^t - 8t^\frac{5}{2}e^t \\
        y(t) = 2C_1te^t + 2tC_2e^t + 10t^\frac{3}{2}e^t - 8t^\frac{5}{2}e^t
    \end{cases}$
\end{Example}
